...
import Input from "./components/ui/input"; // Use lowercase 'input'
import Button from "./components/ui/button"; // Ensure casing matches
...
export default function Home() {
  ...
  const { data: signalData, error: signalError } = useSWR(
    `${API_BASE_URL}/api/v1/signal?ticker=${ticker}`,
    fetcher,
    { refreshInterval: 60000 },
  );
  const { data: historicalData, error: historicalError } = useSWR(
    `${API_BASE_URL}/api/v1/historical?ticker=${ticker}`,
    fetcher,
  );
  ...
  if (signalError || historicalError || portfolioError)
    return <div className="text-red-400">Failed to load data</div>;
  if (!signalData || !historicalData || !portfolioData)
  ...
  return (
    ...
    >
      <div className="lg:col-span-1">
        <form onSubmit={handleSearch} className="mb-6">
          <div className="flex space-x-2">
            <Input
              id="search-ticker" // Add an appropriate id
              type="text"
              value={searchTicker}
      ...
      <div className="lg:col-span-3 space-y-6">
        ...
        <Chart
          data={historicalData.price_history}
